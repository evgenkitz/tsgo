...
## Назначение TypeScript файлов в _packages

Эти файлы не переведены на Go, потому что они служат **интерфейсом для JavaScript/TypeScript потребителей**, а не являются частью основной Go-реализации компилятора.

### Архитектура разделения

### Ключевые причины наличия TypeScript кода

1. **Программный API для JS/TS**: Пакет `@typescript/native-preview` предоставляет программный доступ к компилятору через экспорты в `package.json` package.json:41-87 . Это позволяет использовать компилятор из Node.js приложений.
    
2. **Общие типы AST**: Пакет `_packages/ast` содержит определения типов AST для потребителей API . Эти типы используются как в Go (через генерацию), так и в TypeScript.
    
3. **Интеграция с экосистемой npm**: Проект использует npm workspaces для управления TypeScript пакетами package.json:38-41 , что необходимо для VS Code расширения и других JS-инструментов.
    
4. **Генерация кода**: Часть TypeScript кода генерируется автоматически из Go исходников (например, перечисления в `Herebyfile.mjs`) Herebyfile.mjs:312-327 , что обеспечивает синхронизацию между Go и TypeScript частями.
## Использование
